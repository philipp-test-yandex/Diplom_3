...
class RestorePasswordPage(BasePage):
    ...
    def check_on_forgot_password_page(self):
        ...
        assert self.driver.current_url == FORGOT_PASSWORD_URL, \
            f"[Ошибка!] Ожидался URL восстановления пароля, но был: {self.driver.current_url}"


    @allure.step("Проверка заголовка на странице восстановления")
    def check_header_forgot_password(self):
        header = self.wait.until(EC.visibility_of_element_located(HEADER_FORGOT_PASSWORD))
    ...
    def check_on_reset_password_page(self):
        ...
        assert self.driver.current_url == RESET_PASSWORD_URL, \
            f"[Ошибка!] Ожидался URL '{RESET_PASSWORD_URL}', но был: '{self.driver.current_url}'"
    ...
    def check_password_hidden(self):
        input_type = self.driver.find_element(*NEW_PASSWORD_INPUT_HIDDEN).get_attribute("type")
    ...
    def check_password_visible(self):
        input_type = self.driver.find_element(*NEW_PASSWORD_INPUT_VIS).get_attribute("type")
